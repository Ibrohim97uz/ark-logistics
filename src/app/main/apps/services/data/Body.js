...
import { GetAllServices, DeleteService } from 'hooks';
import { useEffect, useState } from 'react';
import { motion } from 'framer-motion';

import TableBody from './table/TableBody';
import TableHead from './table/TableHead';
import AddNewService from './table/AddNewService';
import Alert from './Alert';

const Body = ({ props, addNewOpen, setAddNewOpen, alertOpen, setAlertOpen }) => {
	const { isLoading, isError, refetch, data } = GetAllServices();
	...
	const goToSinglePage = pageId => {
		props.history.push(`/apps/services/${pageId}`, pageId);
	};
	...
	if (service?.length === 0 && !addNewOpen) {
		return (
			<motion.div
				initial={{ opacity: 0 }}
				animate={{ opacity: 1, transition: { delay: 0.1 } }}
